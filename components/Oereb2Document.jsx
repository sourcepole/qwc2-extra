    ...
    render() {
        ...
        return (
            <div className="oereb-document">
                {this.renderSection("concernedThemes", "oereb.concernedThemes", this.renderConcernedThemes, this.ensureArray(extract.ConcernedTheme))}
                {this.renderSection("notConcernedThemes", "oereb.notConcernedThemes", this.renderOtherThemes, this.ensureArray(extract.NotConcernedTheme))}
                {this.renderSection("themeWithoutData", "oereb.themeWithoutData", this.renderOtherThemes, this.ensureArray(extract.ThemeWithoutData))}
                {this.renderSection("generalInformation", "oereb.generalInformation", this.renderGeneralInformation, extract)}
            </div>
        );
    }
                                ...
                                {Object.entries(subthemedata.symbols).map(([symbol, data], jdx) => {
                                    ...
                                    data.AreaShare ? (
                                        <tr key={"symarea" + jdx}>
                                            ...
                                            {data.PartInPercent ? (<td>{data.PartInPercent.toFixed(1) + "%"}<sup>&nbsp;</sup></td>) : (<td>-</td>)}
                                        </tr>
                                    ) : null];
                                })}
                            </tbody></table>
                            {subthemedata.fullLegend ? (
                                <div>
                                    <div className="oereb-document-toggle-fulllegend" onClick={() => this.toggleFullLegend(fullLegendId)}>
                                        <a><Message msgId={toggleLegendMsgId} /></a>
                                    </div>
